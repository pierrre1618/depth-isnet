...
from data_loader_cache_depth import get_im_gt_name_dict, create_dataloaders, GOSRandomHFlip, GOSResize, GOSRandomCrop, GOSNormalize #GOSDatasetCache,
from basics import  f1_mae_torch #normPRED, GOSPRF1ScoresCache,f1score_torch,
from models import *

def get_gt_encoder(device, train_dataloaders, train_datasets, valid_dataloaders, valid_datasets, hypar, train_dataloaders_val, train_datasets_val): #model_path, model_save_fre, max_ite=1000000):
    ...
    for epoch in range(epoch_num): ## set the epoch num as 100000
        ...
        for i, data in enumerate(gos_dataloader):
            ...
            if ite_num % model_save_fre == 0:  # validate every 2000 iterations
                ...
                tmp_f1, tmp_mae, val_loss, tar_loss, i_val, tmp_time = valid_gt_encoder(device, net, train_dataloaders_val, train_datasets_val, hypar, epoch)
...
def valid_gt_encoder(device, net, valid_dataloaders, valid_datasets, hypar, epoch=0):
    net.eval()
    print("Validating...")
    epoch_num = hypar["max_epoch_num"]
...
def train(device, net, optimizer, train_dataloaders, train_datasets, valid_dataloaders, valid_datasets, hypar,train_dataloaders_val, train_datasets_val): #model_path, model_save_fre, max_ite=1000000):

    if hypar["interm_sup"]:
        print("Get the gt encoder ...")
        featurenet = get_gt_encoder(device, train_dataloaders, train_datasets, valid_dataloaders, valid_datasets, hypar,train_dataloaders_val, train_datasets_val)
    ...
    for epoch in range(epoch_num): ## set the epoch num as 100000
        ...
        for i, data in enumerate(gos_dataloader):
            ...
            if ite_num % model_save_fre == 0:  # validate every 2000 iterations
                notgood_cnt += 1
                net.eval()
                tmp_f1, tmp_mae, val_loss, tar_loss, i_val, tmp_time = valid(device,net, valid_dataloaders, valid_datasets, hypar, epoch)
                net.train()  # resume train
...
def valid(device,net, valid_dataloaders, valid_datasets, hypar, epoch=0):
    net.eval()
    print("Validating...")
    epoch_num = hypar["max_epoch_num"]
...
def main(device,pipe,train_datasets,
         valid_datasets,
    ...
    if(hypar["mode"]=="train"):
        ...
        train_dataloaders, train_datasets = create_dataloaders(train_nm_im_gt_list,
                                                             ...
                                                             my_transforms = [
                                                                             ...
                                                                             GOSResize(hypar["input_size"]),
                                                                              GOSNormalize([0.5,0.5,0.5,0.5],[1.0,1.0,1.0,1.0]),
                                                                              ],
                                                             batch_size = hypar["batch_size_train"],
                                                             shuffle = True,pipe=pipe)
...
if __name__ == "__main__":
    ...
    #depth pipeline (please clean cache database before change)
    ...
    if hypar["mode"] == "train":
        ...
        hypar["model_path"] ="/content/drive/MyDrive/data_train/results_depth" ## model weights saving (or restoring) path
        hypar["restore_model"] = "gpu_itr_49000_traLoss_0.2857_traTarLoss_0.025_valLoss_1.3377_valTarLoss_0.204_maxF1_0.8782_mae_0.0463_time_0.028691.pth" ## name of the segmentation model weights .pth for resume training process from last stop or for the inferencing
        hypar["start_ite"] = 49000 ## start iteration for the training, can be changed to match the restored training process
    ...
    hypar["crop_size"] = [920, 920] ## random crop size from the input, it is usually set as smaller than hypar["cache_size"], e.g., [920,920] for data augmentation
    ...
    hypar["model"] = ISNetDIS(in_ch=4) #U2NETFASTFEATURESUP()
    ...
    main(device,pipe,train_datasets,
         valid_datasets,
         hypar=hypar)